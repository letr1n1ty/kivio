...
- **Sub-agents (`src-tauri/src/agents/` + `chat/sub_agent.rs`)** — named personas (built-in → user `<app_data>/agents/*.md` → project `.kivio/agents/*.md`, later layers override by id) with a system-prompt prefix, optional model override, and a tool allow-list **enforced** at spawn via `filter::filter_tools_for_agent`, which also strips all sub-agent control tools (`agent`/`check_agent_result`/`list_agent_tasks`/`await_agents`) so sub-agents can't recurse or orchestrate. Concurrency is capped by a `SubAgentManager` semaphore (default `DEFAULT_SUB_AGENT_CONCURRENCY` = 12, live-configurable via `settings.chat_tools.sub_agent_concurrency`). The `agent` tool defaults to a **synchronous** spawn (awaited inline, result returned to the parent), but with `background:true` it **detaches** the run into a `tauri::async_runtime::spawn` and returns a `task_id` immediately (dispatch-and-return) so the orchestrator can fan out several agents in one turn. Background results are collected by calling **`await_agents` ONCE** — a runtime-blocking join (waits on `SubAgentManager::terminal_notify`, not a model poll loop) that returns all in-flight background sub-agents' results in a single tool result; it cancels cooperatively on parent-generation bump. The model does **not** poll `check_agent_result` in a loop (polling burns tokens / bloats context; the wait belongs in the runtime); `check_agent_result` / `list_agent_tasks` remain as manual status peeks. Background tasks live only for the spawning run: `SubAgentManager::cancel_run(conversation_id, run_id)` (called at run finalize in `complete_assistant_reply`, and via the generation-cascade on user stop) aborts any still-running detached sub-agents and marks them `Cancelled`, so none outlive their parent run. The registry is in-memory only (results are lost on restart).
