...
fn native_tools_prompt(available_builtin_tools: &[String], language: &str) -> Option<String> {
    ...
    let prompt = if language.starts_with("zh") {
        format!(
            "Kivio 内置工具（已启用）：{list}。只允许调用这里列出的内置工具。read_file 可读取 Kivio 能访问的本地文本文件；write_file、edit_file、run_command 的路径/工作目录仍需在用户主目录内，且可被工作区根目录进一步收紧。write_file、edit_file、run_command 会请求用户确认；run_command 非零退出码代表执行失败，不要用它运行 Skill 自带脚本，Skill 脚本必须走 skill_run_script。run_command 不得用 pip/pip3/python -m pip 安装包来绕过 run_python 沙盒失败；只有用户明确要求修改本机 Python 环境时，才能设置 allow_host_python_package_install=true 且使用 --user 或虚拟环境。run_python 在 Pyodide 沙盒中运行，无本机文件系统和联网能力；导入 numpy、matplotlib、pandas、scipy、sympy、scikit-learn、statsmodels、pillow、seaborn、micropip 等常用包时会自动加载，适合数据运算、统计分析、机器学习基础分析和生成图表；用 run_python 生成图像/图表时，保存为 Pyodide 当前目录下的相对文件名（例如 output.png），不要保存到 /Users 等本机路径，不要 print base64 或 data:image URL；Kivio 会自动捕获并渲染生成的图片。不要在 run_python 里使用 tavily、requests、httpx、urllib3、aiohttp 等联网/API 客户端，{zh_live_access_hint}不要为了这些 Python 包使用 host pip 安装，除非用户明确要求操作本机环境。用户要用 Python 跑代码/计算时优先 run_python，不要用 skill_run_script，除非用户点名某个 Skill。"
        )
    } else {
        format!(
            "Kivio built-in tools enabled: {list}. Only call built-in tools listed here. read_file can read local text files that Kivio can access; write_file, edit_file, and run_command paths/working directories must stay under the user home directory and may be further restricted by workspace roots. write_file, edit_file, and run_command require user approval; run_command treats non-zero exit codes as failures. Do not use run_command to run Skill bundled scripts; use skill_run_script. Do not use pip/pip3/python -m pip through run_command to bypass run_python sandbox failures; only set allow_host_python_package_install=true when the user explicitly asks to modify the host Python environment, and then use --user or a virtual environment. run_python runs in a Pyodide sandbox with no host filesystem or network access and auto-loads common packages when imported, including numpy, matplotlib, pandas, scipy, sympy, scikit-learn, statsmodels, pillow, seaborn, and micropip; use it for data computation, statistical analysis, basic machine-learning analysis, code execution, and charts. When generating images/charts with run_python, save them to relative filenames in the Pyodide current directory such as output.png; do not save to host paths such as /Users, and do not print base64 or data:image URLs. Kivio captures and renders generated images automatically. Do not use network/API clients such as tavily, requests, httpx, urllib3, or aiohttp in run_python; {en_live_access_hint} Do not use host pip to install these Python packages unless the user explicitly asks to modify the host Python environment. For generic Python requests, use run_python—not skill_run_script—unless the user named a specific skill."
        )
    };
    Some(prompt)
}
